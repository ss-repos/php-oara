 ...
 *
 */
class Awin extends \Oara\Network
{
    ...
    public function getTransactionList($merchantList = null, \DateTime $dStartDate = null, \DateTime $dEndDate = null)
    {
        if ($this->_oldAPI) {
			...
			$awinStartDate = clone $dStartDate;
			$awinStartDate->setTime(0, 0, 0);

			$diff = $dEndDate->diff($dStartDate)->format("%a");

			if ($diff <= 30) {
				$awinEndDate = clone $dEndDate;
				$awinEndDate->setTime(23, 59, 59);
			} else { // more than 30 days, go split up in multiple calls, since the Awin API has a max of 31 days
				$awinEndDate = clone $dStartDate;
				$awinEndDate->add(new \DateInterval('P30D'));
				$awinEndDate->setTime(0, 0, 0);
			}
			...
			while ($import) {
				...
				$urls = array();
				$valuesFromExport = array();
				$valuesFromExport[] = new \Oara\Curl\Parameter('startDate', $awinStartDate->format("Y-m-d\TH:i:s"));
				$valuesFromExport[] = new \Oara\Curl\Parameter('endDate', $awinEndDate->format("Y-m-d\TH:i:s"));
				...
				if ($awinEndDate->diff($dEndDate)->format("%r%a") <= 0) { // we have collected all transactions
					$import = false;
				} else {
					$awinStartDate->add(new \DateInterval('P30D'));
					$awinEndDate->add(new \DateInterval('P30D'));

					if ($awinEndDate->diff($dEndDate)->format("%r%a") < 0) { // make sure we do not get transactions later than endDate
						$awinEndDate = clone $dEndDate;
						$awinEndDate->setTime(23, 59, 59);
					}
